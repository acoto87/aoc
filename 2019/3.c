...
void printSegment(struct segment s)
{
    printf("%d,%d -> %d,%d\n", s.x1, s.y1, s.x2, s.y2);
}

void printSegments(struct segment s[], int length)
{
    printf("%d\n", length);
...
int getLength(struct segment s)
{
    return getDistance(s.x1, s.y1, s.x2, s.y2);
}

struct segment reorder(struct segment s)
{
    struct segment r;
    if (s.x1 == s.x2)
    {
...
int intersect(struct segment s1, struct segment s2, int* x, int* y)
{
    s1 = reorder(s1);
    s2 = reorder(s2);
...
void getSegments(const char* path, struct segment segments[], int* numberOfSegments)
{
    int x = 0;
    int y = 0;
    int n = 0;
    ...
    while (1)
    {
        ...
        if (n > 0)
        {
            segments[n].x1 = segments[n - 1].x2;
            segments[n].y1 = segments[n - 1].y2;
        }
...
int part1()
{
    ...
    {
        struct segment s1 = segments1[i];

        for (int j = 0; j < length2; j++)
        {
            struct segment s2 = segments2[j];

            int xi, yi;
            if (intersect(s1, s2, &xi, &yi))
            {
                int dist = getDistance(0, 0, xi, yi);
...
int part2()
{
    ...
    for (int i = 0; i < length1; i++)
    {
        ...
        for (int j = 0; j < length2; j++)
        {
            ...
            if (intersect(s1, s2, &xi, &yi))
            {
                int steps = steps1 + getDistance(s1.x1, s1.y1, xi, yi) +
                            steps2 + getDistance(s2.x1, s2.y1, xi, yi);
