...
struct instruction decode(int instruction)
{
    return (struct instruction){ instruction % 100, { (instruction / 100) % 10, (instruction / 1000) % 10, (instruction / 10000) % 10 } };
}

int getArg(int* memory, int pointer, int param, int mode)
{
    return mode == 0
...
int runProgram(int* memory, int length, int input)
{
    int output = 0;
    ...
    while (pointer < length)
    {
        ...
        struct instruction inst = decode(instruction);
        ...
        if (inst.opcode == 1)
        {
            int arg0 = getArg(memory, pointer, 0, inst.modes[0]);
            int arg1 = getArg(memory, pointer, 1, inst.modes[1]);
            int arg2 = getArg(memory, pointer, 2, 1);

            memory[arg2] = arg0 + arg1;
        ...
        {
            int arg0 = getArg(memory, pointer, 0, 1);
...
int startProgram(int input)
{
    int length = arrayLength(program);

    int* memory = (int*)malloc(length * sizeof(int));
    memcpy(memory, program, length * sizeof(int));

    int result = runProgram(memory, length, input);
